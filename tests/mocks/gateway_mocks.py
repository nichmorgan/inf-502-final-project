...
def mock_gateway(mocker: MockerFixture, faker: Faker) -> RepoPort:
    ...
    gateway.get_closed_pull_requests_count.return_value = faker.random_int(
        min=0, max=500
    )
...
def mock_gateway_with_timeseries(mocker: MockerFixture, faker: Faker) -> RepoPort:
    ...
    timeseries_open = {
        base_date + timedelta(days=i): faker.random_int(min=0, max=50)
        for i in range(10)
    }
    timeseries_closed = {
        base_date + timedelta(days=i): faker.random_int(min=0, max=30)
        for i in range(10)
    }
    timeseries_users = {
        base_date + timedelta(days=i): faker.random_int(min=1, max=20)
        for i in range(10)
    }
